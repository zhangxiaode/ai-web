<template>
  <div class="h-100% px-32px overflow-hidden flex flex-col items-normal">
    ...
    >
      <n-form-item path="name" label="智能体名称">
        <n-input v-model:value="searchForm.name" size="small" placeholder="请输入智能体名称" @keyup.enter.native="getList" />
      </n-form-item>
      <n-form-item>
    ...
    <div class="flex-1 overflow-auto agent-wrap" v-loading="loading">
      <div v-for="(item, index) in agent_list" :key="index" class="h-30px float-left p-12px rounded-8px bg-#252525 flex flex-between items-center">
        ...
        <div class="flex justify-between items-center">
          ...
          <n-button class="mx-6px" type="primary" size="tiny"  @click="handleCreateAgent(item)">
            <template #icon>
              <n-icon>
                <CreateOutline />
              </n-icon>
          ...
          <n-button class="mx-6px" type="primary" size="tiny"  @click="createAgentVideo(item)">
            <template #icon>
              <n-icon>
                <CreateOutline />
              </n-icon>
    ...
    />
    <NewAgentModal @save="handleNewModalComplete" />
    <AgentVideoModal @save="handleAgentVideoModalComplete" />
  </div>
</template>
<script lang="ts" setup>
import { useModal } from "@/hooks";
import { Search, Repeat, AddSharp, CreateOutline, TrashOutline } from '@vicons/ionicons5';
import { getAgentList, createAgent, deleteAgent } from '@/apis/index';
import AudioPlayer from '@/components/audioPlayer.vue';
import NewAgentModal from './components/newAgentModal.vue';
import AgentVideoModal from './components/agentVideoModal.vue';
...
const size = ref(10)
const total = ref(0)
const agent_list: any = ref([])
const { showModal: showNewModal } = useModal("new-modal");
const { showModal: showAgentVideoModal } = useModal("agent-video-modal");
...
const handleCreateAgent = (item: any) => {
  dialog.warning({
    title: '温馨提示',
    content: () => '确定要生成该克隆人吗？',
    positiveText: '确定',
    ...
    onNegativeClick: () => {
      message.warning('已取消生成克隆人')
    }
  })
}
const createAgentVideo = (item: any) => {
  showAgentVideoModal({
    id: item.id
  });
}
const handleAgentVideoModalComplete = () => {
  getList();
};
...
const getList = async () => {
  ...
  try {
    ...
    agent_list.value = res.data.rows
  } catch (error) {
    console.log(error)
  }
  loading.value = false
