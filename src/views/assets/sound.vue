<template>
  <div class="h-100% px-32px overflow-hidden flex flex-col items-normal">
    ...
    <div class="flex-1 overflow-auto sound-wrap" v-loading="loading">
    ...
    />
    <NewSoundModal @save="handleNewModalComplete" />
  </div>
</template>
...
const getList = async () => {
  ...
  try {
    ...
    sound_list.value = res.data.rows
  } catch (error) {
    console.log(error)
  }
  loading.value = false
