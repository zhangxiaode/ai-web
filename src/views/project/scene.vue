<template>
  <div class="h-100% px-32px overflow-hidden flex flex-col items-normal">
    ...
    >
      <n-form-item path="name" label="场景名称">
        <n-input v-model:value="searchForm.name" size="small" placeholder="请输入场景名称" @keyup.enter.native="getList()" />
      </n-form-item>
      <n-form-item>
    ...
    <div class="flex-1 overflow-auto scene-wrap" v-loading="loading">
      <div v-for="(item, index) in list" :key="index" class="h-30px float-left p-12px rounded-8px bg-#252525 flex flex-between items-center">
    ...
    />
    <NewSceneModal @save="handleNewModalComplete" />
  </div>
</template>
<script lang="ts" setup>
import { useModal } from "@/hooks";
import { Search, Repeat, AddSharp, CreateOutline, TrashOutline } from '@vicons/ionicons5';
import { getSceneList, deleteScene } from '@/apis/index';
import NewSceneModal from './components/newSceneModal.vue';
