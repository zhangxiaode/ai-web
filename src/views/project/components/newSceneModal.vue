<template>
  <n-modal v-model:show="visible" mask-closable preset="dialog" :show-icon="false" class="dialog"
    ...
    <template #header>
      <slot name="header">{{ form.id ? '编辑' : '新增' }}场景</slot>
    </template>
    <slot>
      <div class="new-content">
        ...
        >
          ...
          <n-form-item label="场景描述:" path="desc">
            <n-input v-model:value="form.desc" placeholder="请输入场景描述" />
          </n-form-item>
...
const rules = {
  name: {required: true, message: "场景名称不能为空", trigger: ['blur', 'change']},
  resource_path: {required: true, message: "场景图片不能为空", trigger: ['blur', 'change']}
};
