      ...
      <div class="audio-book flex flex-col items-normal" v-loading="loading">
        <div class="w-100% flex flex-col justify-between items-normal">
              ...
              <div v-for="(ele, idx) in scene.elements" :key="idx" class="bg-555 my-5px p-8px">
                ...
                <div class="flex items-center justify-between my-5px">
                  ...
                  <div class="flex-1 text-14px c-#fff">
                    ...
                    emotion_scale(1~5):<n-input-number v-model:value="ele.emotion_scale" :bordered="false" :show-button="false" placeholder="请输入" />
                    speech_rate([-50,100]):<n-input-number v-model:value="ele.speech_rate" :bordered="false" :show-button="false" placeholder="请输入" />
                    loudness_rate([-50,100]):<n-input-number v-model:value="ele.loudness_rate" :bordered="false" :show-button="false" placeholder="请输入" />
                    pitch([-12,12]):<n-input-number v-model:value="ele.pitch" :bordered="false" :show-button="false" placeholder="请输入" />
                ...
                <div v-else class="flex items-center justify-between my-5px">
                  ...
                  <div class="flex-1 text-14px c-#fff flex items-center">
                    ...
                    音调，取值范围[-12,12]，整数，不带双引号，12代表升高一个八度，-12代表降低一个八度 -->
          ...
          <div class="leadings-[1.5em] flex flex-wrap px-12px mt-12px">
            ...
            <div class="w-100% text-12px c-#696969 mt-8px">字符数 => 字数计算规则（汉字：2字符、英文字母/数字/标点/空格：1字符、计算文本长度时，SSML标签内容也包含在内）(小于5秒按5秒计费)</div>
          </div>
        </div>
      </div>
    </slot>
...
const onTransformAudio = async (ele: any) => {
  ...
  try {
    const res: any = await postScriptAudio({
      ...
      role_id: ele.role_id,
      content: ele.content_mark || ele.content,
      ...
      pitch: ele.pitch // 音调，取值范围[-12,12]，整数，不带双引号，12代表升高一个八度，-12代表降低一个八度
    })
