      ...
      <div class="create-content" v-loading="loading">
        ...
        >
          ...
          <n-form-item v-if="form.model === 'cyzx-image-d-seedream' || form.model === 'cyzx-image-q-edit' || form.model === 'cyzx-image-q-i2i'" label="上传图片:" path="images">
            <UploadObs :accept="suffix_accept" :max="form_rules.input_images_max || 1" :size_max="form_rules.input_image_size_max" :get_file_path="({ file_name }) => `novel/${route.query.id}/scene/${file_name}`" @change="({ resource_path }) => form.images = resource_path.map((item: any) => item.original_url)" />
          </n-form-item>
        </n-form>
      </div>
    </slot>
...
import { createScene, getOptions } from "@/apis/index";
  ...
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      ...
      try {
        ...
        delete params['size'];
        const res: any = await createScene(params)
