<template>
	<n-modal v-model:show="visible" mask-closable preset="dialog" :show-icon="false" class="dialog" style="width: 600px;" @update:show="onClose">
		<template #header><slot name="header">绑定手机号</slot></template>
		<slot>
			<n-form
			...
			>
				<n-form-item label="手机号:" path="phone">
					<n-input v-model:value="form.phone" placeholder="请输入你的手机号" />
				</n-form-item>
...
import { getCaptcha, sendCode, bindPhone } from "@/apis/index";

const emit = defineEmits(["save"]);
const { visible, hideModal } = useModal('bind-phone-modal');
...
const onSubmit = async () => {
	formRef.value?.validate(async (errors) => {
		if (!errors) {
			...
			try {
				const res: any = await bindPhone(params)
				if (res.code == 200 || res.code == 0) {
					onClose()
					emit('save', {
