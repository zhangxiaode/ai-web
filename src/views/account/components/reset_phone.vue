<template>
	<n-modal v-model:show="visible" mask-closable preset="dialog" :show-icon="false" class="dialog" style="width: 600px;" @update:show="onClose">
		<template #header><slot name="header">修改手机号</slot></template>
		<slot>
			<n-form
			...
			>
				...
				<n-form-item label="新手机号:" path="newPhone">
					<n-input v-model:value="form.newPhone" placeholder="请输入新的手机号" />
				</n-form-item>
			</n-form>
		</slot>
		<template #action>
...
import { getCaptcha, sendCode, updatePhone } from "@/apis/index";

const emit = defineEmits(["save"]);
const { visible, payload, hideModal } = useModal('reset-phone-modal');
...
const rules = {
	...
	newPhone: [
		{required: true, message: "新手机号不能为空", trigger: ['blur', 'change']},
		{pattern: /^1[3-9]\d{9}$/, message: "新手机号格式错误", trigger: ['blur', 'change']}
	],
	code: [
...
const onSubmit = async () => {
	formRef.value?.validate(async (errors) => {
		if (!errors) {
			...
			try {
				const res: any = await updatePhone(params)
				if (res.code == 200 || res.code == 0) {
					onClose()
					emit('save', {
						phone: res?.data?.newPhone
					})
				}
			} catch (error) {
				console.log(error)
