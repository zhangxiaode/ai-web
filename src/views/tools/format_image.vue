<template>
  <div class="h-100% overflow-auto">
    <div class="p-32px flex flex-col justify-center items-center">
      <!-- 图片格式转换 -->
      <Upload accept="image/png, image/jpg, image/jpeg, image/ico, image/gif, image/bmp, image/tif, image/tga, image/apng, image/pcx" :max="1" :size_max="20" folder="image" @change="handleUploadChange" />
      ...
      <div class="flex items-center justify-center">
        <div v-if="image_url" class="w-150px h-50px leading-50px mx-12px px-16px rounded-25px bg-#db2777 text-center c-#fff text-14px cursor-pointer" @click="handleFormat()">格式转换</div>
        <div v-if="image_url" class="w-150px h-50px leading-50px mx-12px px-16px rounded-25px bg-#db2777 text-center c-#fff text-14px cursor-pointer" @click="handleDownload()">下载</div>
      </div>
    </div>
  </div>
</template>
...
const handleUploadChange = async ({ resource_path }) => {
  if(resource_path.length > 0) {
    image_url.value = resource_path.map((item: any) => item.original_url)[0]
